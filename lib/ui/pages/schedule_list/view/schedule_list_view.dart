    ...
    return BlocBuilder<ScheduleListBloc, ScheduleListState>(
      ...
      builder: (context, state) => Expanded(
        child: BaseList(
          ...
          onRefresh: () => context.read<ScheduleListBloc>().add(ScheduleListFetch()),
          itemBuilder: (context, item) => ScheduleItem(item),
        ),
      ),
    );
  }
...
class ScheduleItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BaseCard(
      ...
      child: Text(scheduling.day != null ? DateFormat(DateFormat.YEAR_MONTH_DAY).format(scheduling.day!) : ''),
    );
  }
}
